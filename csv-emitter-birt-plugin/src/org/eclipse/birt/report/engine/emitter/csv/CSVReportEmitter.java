...
{		
	...
	public void start( IReportContent report )
	{
		...
		if(tableToOutput == null)
			this.tableToOutput=System.getProperty(ICSVRenderOption.EXPORT_TABLE_BY_NAME, "Default");
		...
		if(delimiter == null)
			delimiter = System.getProperty(ICSVRenderOption.DELIMITER, CSVTags.TAG_COMMA);
		...
		// Setting Default value to false if user has not specified any value
		if(showDatatypeInSecondRow == null)
		{
			showDatatypeInSecondRow = Boolean.parseBoolean(
					System.getProperty(ICSVRenderOption.SHOW_DATATYPE_IN_SECOND_ROW, "false"));
		}
		
		protectedChars = (String)renderOption.getOption(ICSVRenderOption.PROTECTED_CHARS);
		
		// Setting Default Field Delimiter if user has not specified any Delimiter
		if(protectedChars == null)
			protectedChars = System.getProperty(ICSVRenderOption.PROTECTED_CHARS, CSVTags.TAG_COMMA + CSVTags.TAG_CR);
		
		quoteChar = (String)renderOption.getOption(ICSVRenderOption.QUOTE_CHAR);
		
		// Setting Default Field Delimiter if user has not specified any Delimiter
		if(quoteChar == null)
			quoteChar = System.getProperty(ICSVRenderOption.QUOTE_CHAR, CSVTags.TAG_QUOTE);
	...
	{				
		...
		if ( writeData )
		{
			writer.text( textValue, protectedChars, quoteChar );
		}
	}
	...
	{		
		...
		{			
			...
			for(int i=0;i<columnNamesInTableOrder.size();i++)
			{
				...
				if(dataType != null)
					writer.text(dataType, protectedChars, quoteChar);
